...
export default function ToDoList() {
  ...
  function handleEnter(event) {
    if (newItemInput.trim()) {
      if (event.key === 'Enter') {
        ...
        setNewItemInput('') //input temizliyor
        scrollToBottom()
      }
    }
  }
  ...
  function autoComplete() {
    setAutoCompleteRequested(true) //Otomatik tamamla başlatıyor
  }
  ...
  onFocus={toggleInputFocus} olarak kullanabilirdim ama burada onBlur kullandım.
  */
      ...
      <div className="to-do-list-item-container" key={item.id}>
        <label className="checkbox-label">
          <input
            ...
            checked={item.complete} //Checkbox'ı işaretleme
          />
      ...
      <div className="to-do-list-container" ref={listContainerRef}>
        ...
        <label className="new-item-label">
          <img
            ...
            className={`add-item-icon ${inputInFocus && 'faded'}`} // Eğer input focus'ta ise 'faded' class'ı ekleniyor`}
          />
          <input
            ...
            onFocus={() => setInputInFocus(true)} // Input focus'ta
            onBlur={() => setInputInFocus(false)} // Input focus'tan çıkıldığında
            value={newItemInput} // Input'un değerini kontrol ediyor
          />
        </label>
      </div>
...
/*
  ...
  onFocus sadece input elementi focus olduğunda tetiklenir ve toggleInputFocus state’i değiştirir (yani inputInFocus state’ini true yapar).

  Fakat, focus kaybolduğunda (input dışına tıkladığınızda) onBlur olmadığı için, inputInFocus state’ini tekrar false yapmak mümkün olmaz. Yani, input focus’tan çıktığında bile state true olarak kalır ve görselin opaklığı hala düşük olabilir.

  Bu yüzden, hem focus hem de focus dışına çıkma durumunu kontrol edebilmek için onFocus ve onBlur event’lerini beraber kullanmak en iyi yaklaşım olacaktır.

  Tek başına onFocus kullanmak, input focus’a geçtiğinde görselin opaklığını ayarlayabilir ancak focus’tan çıktığında geri döndüremez.
